...
with open(fileName, 'r') as file:
    ...
    for row in csvreader:
        ...
        # previous year price
        predictedDate = fiscalDateEnding + relativedelta(years=-1)

        dateKey = f"{predictedDate.year}-{predictedDate.month}"
        mappedDateKey = priceMapping[dateKey]

        thisYearDateKey = f"{fiscalDateEnding.year}-{fiscalDateEnding.month}"
        thisYearMappedDateKey = priceMapping[thisYearDateKey]
        try:
            lastYearPrice = priceDict[symbol][mappedDateKey]["4. close"]
            thisYearPrice = priceDict[symbol][thisYearMappedDateKey]["4. close"]
            diff = float(thisYearPrice) - float(lastYearPrice)
            yoy = diff / float(lastYearPrice)
            yoy = round(yoy, 7)

            updatedRow = row
            updatedRow.append(yoy)
